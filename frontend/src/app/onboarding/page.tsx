...
// Flag paths updated — spaces removed from filenames
const MATCH = {
...
export default function OnboardingPage() {
  ...
  // Dev bypass: navigate to /onboarding?dev=1
  useEffect(() => {
    const isNewUser = sessionStorage.getItem('predensity-new-user') === 'true';
    const devBypass = sessionStorage.getItem('predensity-dev-onboarding') === 'true';
    const urlBypass = typeof window !== 'undefined' && window.location.search.includes('dev=1');
    if (!isNewUser && !devBypass && !urlBypass) {
      router.replace('/markets');
      return;
    }
    sessionStorage.removeItem('predensity-new-user');
    sessionStorage.removeItem('predensity-dev-onboarding');
    setIsAllowed(true);
  }, [router]);
  ...
  return (
    <motion.div
      ...
      className="min-h-screen overflow-hidden relative flex flex-col font-sans bg-[#1e3a5f]"
    >
      <button
            ...
            >
              ...
              <AnimatePresence>
                {prevIndex !== null && prevIndex !== activeIndex && (
                  ...
                  >
                    <Image src={CATEGORIES[prevIndex].image} alt="" width={32} height={32} className="w-8 h-8 object-contain opacity-40" />
