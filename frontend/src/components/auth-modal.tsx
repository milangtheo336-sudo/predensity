        ...
        <div className="p-8">
          {view === 'main' ? (
            ...
            <>
              ...
              <div className="mb-5">
                <button onClick={() => setView('main')} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors mb-4">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {eip6963Wallets.length > 0
                    ? `${eip6963Wallets.length + 1} wallet${eip6963Wallets.length > 0 ? 's' : ''} available`
                    : 'Detecting wallets...'}
                </p>
              </div>
              ...
              <div className="space-y-2 max-h-[360px] overflow-y-auto pr-1">
                ...
                >
                  {lastUsedMethod === 'hashpack' && (
                    <span className="absolute -top-2 -right-2 px-2 py-0.5 bg-gray-700 text-gray-300 text-[10px] font-semibold rounded-full border border-white/10">Last used</span>
                  )}
                  <div className="flex items-center gap-4">
                    <Image src="/hashpack.jpg" alt="HashPack" width={40} height={40} className="rounded-lg object-contain flex-shrink-0 w-10 h-10" />
                    <div className="text-left">
                      <div className="font-medium">HashPack</div>
                      <div className="text-xs text-gray-500">Hedera native</div>
                    </div>
                  </div>
                  <span className="text-xs text-gray-500 flex-shrink-0">Hedera</span>
                </button>

                {/* EIP-6963 discovered wallets — icons and names come from the wallet itself */}
                {eip6963Wallets.map((w) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <img src={w.info.icon} alt={w.info.name} className="rounded-lg object-contain flex-shrink-0 w-10 h-10" />
                      <div className="text-left">
                        <div className="font-medium">{w.info.name}</div>
                        <div className="text-xs text-gray-500">{w.info.rdns}</div>
                      </div>
                    </div>
                    <span className="text-xs text-gray-500 flex-shrink-0">Detected</span>
                  </button>
                ))}
